...
export function AdminDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(
        ([summaryData, membersResult, eventsResult, transactionsResult]) => {
          setSummary({
            ...summaryData,

            totalMembers: membersResult.data.length,

            activeVolunteers: membersResult.data.filter(
              (member) => member.isActive,
            ).length,
            upcomingEvents: eventsResult.data.length,
            monthlyBirthdays: membersResult.data.filter((member) =>
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <span className="px-4 py-2 text-sm font-bold text-blue-700 rounded-full bg-blue-50">
          {loading ? "Carregando API..." : "Dados sincronizados com API"}
        </span>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <StatCard
          ...
          // helper="↑ 3,4% vs mês anterior"
        />
        <StatCard
          icon={UserCheck}
          title="Membros ativos"
          value={data.activeVolunteers.toLocaleString("pt-BR")}
          // helper="↑ 7,2% vs mês anterior"
        />
        <StatCard
          icon={CalendarDays}
          title="Próximos eventos"
          value={String(data.upcomingEvents)}
          helper="2 eventos esta semana"
        />
        <StatCard
          icon={DollarSign}
          title="Arrecadações (mês)"
          value={`R$ ${data.monthlyIncome.toLocaleString("pt-BR")}`}
        ...
        <StatCard
          ...
          value={`R$ ${data.pendingExpenses.toLocaleString("pt-BR")}`}
          // helper="↓ 4,3% vs mês anterior"
          tone="red"
        />
        <StatCard
          icon={Cake}
          title="Aniversariantes"
          value={String(data.monthlyBirthdays)}
          // helper="6 esta semana"
          tone="amber"
        />
      </div>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {events.slice(0, 5).map((event) => (
              <div
                className="p-4 border rounded-2xl border-slate-100"
                key={event.id}
              >
        ...
        <Card>
          ...
          <div className="grid gap-3 mt-4 sm:grid-cols-2">
            {members.slice(0, 4).map((member) => (
              <div className="p-3 rounded-2xl bg-slate-50" key={member.id}>
