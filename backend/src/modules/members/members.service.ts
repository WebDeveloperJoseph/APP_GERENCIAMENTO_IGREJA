...
interface AuthenticatedMember {
  ...
  churchId: string; // 🔑 Adicionado ao contrato do usuário autenticado
}
...
class MembersService {
  ...
  async list(churchId: string) {
    return prisma.member.findMany({
      where: {
        isActive: true,
        churchId, // 🛡️ Filtrado estritamente pela igreja logada
      },
      orderBy: {
        createdAt: "desc",
      },
  ...
  async show(id: string, churchId: string) {
    const member = await prisma.member.findFirst({
      where: {
        id,
        churchId, // 🔑 Multi-Tenant: Garante que o membro buscado pertence à igreja do usuário ativo
      },
      select: memberSelect,
    });

    if (!member) {
      throw new AppError("Membro não encontrado nesta igreja.", 404);
    }
  ...
  async create(input: MemberInput, authenticatedMember: AuthenticatedMember) {
    ...
    return prisma.member.create({
      data: {
        ...
        churchId: authenticatedMember.churchId, // 🔑 Multi-Tenant: O novo membro já nasce amarrado à igreja do criador
      },
      select: memberSelect,
    });
  }
  ...
  async put(
    ...
    // 🔑 Multi-Tenant: Valida se o membro pertence à mesma igreja
    const memberExists = await prisma.member.findFirst({
      where: {
        id,
        churchId: authenticatedMember.churchId,
      },
    });
  ...
  async delete(id: string, authenticatedMember: AuthenticatedMember) {
    ...
    // 🔑 Multi-Tenant: Verifica posse da igreja
    const memberExists = await prisma.member.findFirst({
      where: {
        id,
        churchId: authenticatedMember.churchId,
  ...
  async deletePermanently(
    ...
    // 🔑 Multi-Tenant: Verifica posse da igreja antes do hard delete
    const memberExists = await prisma.member.findFirst({
      where: {
        id,
        churchId: authenticatedMember.churchId,
  ...
  async listInactive(churchId: string) {
    return prisma.member.findMany({
      where: {
        isActive: false,
        churchId, // 🔑 Multi-Tenant: Lista inativos apenas da igreja correspondente
      },
      orderBy: {
        updatedAt: "desc",
      },
  ...
  async restore(id: string, authenticatedMember: AuthenticatedMember) {
    ...
    // 🔑 Multi-Tenant: Verifica se o membro inativo pertence a esta igreja
    const memberExists = await prisma.member.findFirst({
      where: {
        id,
        churchId: authenticatedMember.churchId,
